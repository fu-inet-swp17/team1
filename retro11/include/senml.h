...
typedef enum {
    ...
    SENML_TYPE_INT    = 1,  /**< Indicates an integer value                */
...
typedef struct {
    ...
    int32_t             base_time;       /**< Update time values are relative to this time          */
    char               *base_unit;       /**< When all values have the same unit, set this field    */
    int32_t             base_value;      /**< If not zero, add this to (float) value in each record */
    int32_t             base_sum;        /**< If not zero, add this to sum in each record           */
} senml_base_info_t;
...
typedef struct {
    ...
    int32_t            time;        /**< Time (seconds since unix epoch) of the measurement       */
    int32_t            update_time; /**< Time before sensor provides an updated measurement       */
    int32_t            value_sum;   /**< Integrated sum of the values over time                   */
    senml_value_type_t value_type;  /**< Indicates which type the value is of                     */
    union {
        int32_t           f;  /**< An integer value                           */
